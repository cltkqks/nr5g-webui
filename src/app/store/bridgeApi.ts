  ...
  endpoints: (build) => ({
    connect: build.query<{ ok: true }, { url: string } | void>({
      ...
      async onCacheEntryAdded(arg, api) {
        ...
        let EVENT_LOG_LIMIT = 60; // mirrored from analyzerSlice constants
        try {
          const ws = new WebSocket(url);
          wsMap.set(url, ws);

          ws.onopen = () => {
            const prev = (api.getState() as any).analyzer;
            dispatch(
          ...
          ws.onmessage = (ev) => {
            ...
            const state = (api.getState() as any).analyzer;
            switch (msg.type) {
              case "heartbeat": {
              ...
              case "state": {
                const patch = msg.payload as any;
          ...
          ws.onerror = (ev) => {
            const prevErr = (api.getState() as any).analyzer;
            dispatch(
              analyzerActions.applyPatch({
                eventLog: appendWithLimit(
                  prevErr.eventLog,
          ...
          ws.onclose = () => {
            const prevClose = (api.getState() as any).analyzer;
            dispatch(
              analyzerActions.applyPatch({
    ...
    updateConfig: build.mutation<{ ok: true }, { url: string; patch: any }>(
      {
        async queryFn({ url, patch }) {
          const ws = wsMap.get(url);
          if (!ws || ws.readyState !== WebSocket.OPEN) {
            return {
              error: { status: "WS_CLOSED", error: "WebSocket not open" },
